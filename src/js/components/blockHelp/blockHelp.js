...
main.insertAdjacentHTML('beforeend', blockHelp_blockHelpTemplate());
...
function openModal(modal, buttonClose, onButtonCloseModalClick, submitButton, onSubmitClick, inputEl){
 ...
 // close modal
 function closeHelpModal(buttonClose, onButtonCloseModalClick, modal, submitButton, onSubmitClick, inputEl){
...
function onButtonPhoneClick(event){
    openModal(helpPhoneModal, buttonClosePhoneModal, onButtonClosePhoneModalClick, phoneSubmitButton, onPhoneSubmitClick, phoneInput);    
};

 function onButtonClosePhoneModalClick(event){
    closeHelpModal(buttonClosePhoneModal, onButtonClosePhoneModalClick, helpPhoneModal, phoneSubmitButton, onPhoneSubmitClick, phoneInput);
...
function onButtonChatClick(event){
    openModal(helpChatModal, buttonCloseChatModal, onButtonCloseChatModalClick, messageSubmitButton, onMessageSubmitClick, messageInput);    
};
 function onButtonCloseChatModalClick(event){
    closeHelpModal(buttonCloseChatModal, onButtonCloseChatModalClick, helpChatModal, messageSubmitButton, onMessageSubmitClick, messageInput);   
 };
 // messageModal submit
 function onMessageSubmitClick(event){
   if(messageInput.validity.valid){
    ...
    messageThumb.insertAdjacentHTML('beforeend', `<p class="help__chat-message client-message">${clientSendedMessage}</p>`);
    messageInput.value = "";
   }     
 };
 //count messages
...
//======================user not active 
  ...
  function stopCounter(){
    setTimeout(() => { clearInterval(counterId) }, 1000);
